...
from UNetCFG.blocks import ResBlock, DownBlock, UpBlock, SelfAttention, TimeEmbedding
from UNetCFG.alphabeta import compute_linear_beta_schedule, compute_alpha_schedule, compute_alpha_cumulative_product
import math
import os
...
class UNet(nn.Module):
    def __init__(self, dim = 32, in_channels=3, out_channels=3, time_dim=256, num_classes = None, device="cuda"):
        ...
        if num_classes is not None:
            self.label_emb = nn.Embedding(num_classes, self.time_projection_dim)
    ...
    def forward(self, x, t, l):
        ...
        if l is not None:
            #print (f"\nlabel shape: {self.label_emb(l).shape}, t_emb shape: {t_emb.shape}")
            t_emb += self.label_emb(l)
...
def calc_loss(u_net, x, labels, timesteps=1000):
    ...
    predicted_noise = u_net(x_t, t/timesteps,labels)
...
def generate_samples(u_net, nsamples, image_shape, labels, cfg_scale=3, timesteps=1000):
    # Define the linear beta schedule
    betas = linear_beta_schedule(timesteps)
    ...
    for t in reversed(range(timesteps)):
        ...
        # Predict the noise
        predicted_noise = u_net(x_t, t_tensor/timesteps, labels).detach()
        
        if cfg_scale > 0:
            uncond_predicted_noise = u_net(x_t, t_tensor/timesteps, None)
            predicted_noise = torch.lerp(uncond_predicted_noise, predicted_noise, cfg_scale).detach()
