...
from UNetCFG.model import generate_samples, calc_loss
...
def train_model(u_net, dataset, config, model_name, log=False, save_model=False):
    ...
    save_dir = os.path.join("UNetCFG/results", model_name)  # Create the full directory path
    ...
    for i_epoch in range(config["epochs"]):
        ...
        for batch_idx, (data, labels) in enumerate(progress_bar):
            ...
            if np.random.random() < 0.1:
                labels = None
            loss = calc_loss(u_net, data, labels)
            loss.backward()
            opt.step()
        ...
        torch.save(u_net.state_dict(), f"UNetCFG/models/{model_name}ckpt.pt")

        if (i_epoch < 5 or i_epoch % 5 == 0):
            print("generating samples")
            # Generate 8 samples after each epoch
            labels = torch.arange(10).long().to(device)
            generated_samples = generate_samples(u_net, nsamples=10, image_shape=image_shape, labels = labels, timesteps=1000)
